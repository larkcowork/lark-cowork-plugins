...
| CRM | `~~CRM` | HubSpot, Close (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
| Data enrichment | `~~data enrichment` | Clay, ZoomInfo, Apollo (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
...
| Meeting transcription | `~~conversation intelligence` | Lark Minutes + VC (`lark`) | `lark_minutes_search`, `lark_vc_search` |
| Project tracker | `~~project tracker` | Lark Task + Base (`lark`) | `lark_task_create`, `lark_task_my`, `lark_base_search` |
| Sales engagement | `~~sales engagement` | Outreach (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
