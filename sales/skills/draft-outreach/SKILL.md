...
> Web research + external enrichment **stay as-is**. The collaboration layer is Lark: **CRM = Lark Base** prior-relationship check (`lark_base_search`, requires `search_fields`; narrow with `select_fields` — base_search does NOT support `jq`, P5); resolve a known recipient to a real `open_id` via `lark_contact_search` (P1). **Email = Lark Mail, drafts only** — always `lark_mail_draft_create` so the rep eyeballs it in the Mail UI; never auto-send (`lark_mail_send` requires `confirm_send: true`, P2). For re-engaging accounts that have gone dark, **delegate to the installed `client-followup` skill** (detects dormant CRM contacts >21d and drafts personalized re-engagement mail — drafts only). For deep mail/folder/draft control, delegate to **`lark-mail`**.
...
[Draft created in Lark Mail - review and send (never auto-sent)]
[Lark Mail not connected - copy email above]
...
**Use the account-research skill internally:**
```
1. Web search for company + person
2. If Enrichment connected (external tool): verified contact info, background
3. CRM = Lark Base (P5): lark_base_search(base_token, table_id, query="<person/company>",
     search_fields=["Account Name"],            # REQUIRED — field(s) to match on
     select_fields=["Account Name","Stage","Last Touch","Notes"], limit=5)
   — check for prior relationship; base_search does NOT support jq.
   If field names unknown, discover via
   lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields.
   resolve a known teammate/contact → open_id via lark_contact_search (P1)
```
...
```
Email = Lark Mail (drafts only, P2):
1. lark_mail_draft_create(to, subject, body)   — preview the payload with dry_run first
2. Return the draft so the rep reviews it in the Mail UI
3. Note: "Draft created — review in Lark Mail, then send"
   NEVER auto-send. lark_mail_send needs confirm_send:true and explicit approval.

If Lark Mail not available:
1. Output the email text
...
**Input:** "draft outreach to the Head of Engineering at Lark Wiki"

**Research finds:**
- Name: David Tibbitts
- Recent news: Lark Wiki launched AI features
...
```markdown
# Outreach Draft: David Tibbitts @ Lark Wiki

## Research Summary
**Target:** David Tibbitts, Head of Engineering at Lark Wiki
...
**To:** david@notion.so
**Subject:** Lark Wiki's AI scaling + a thought

---

Hi David,

Saw Lark Wiki's AI rollout is gaining serious traction - congrats.
...
1. Lark Wiki AI + scaling question
2. Quick thought on Lark Wiki's ML hiring

---

## Email Draft Status
Draft created in Lark Mail (lark_mail_draft_create) - review and send. Never auto-sent (P2).
```
...
- **client-followup** — installed dormant-account re-engagement (delegate for "went dark" outreach)
- **account-research** — Deep prospect research before drafting
- **lark-mail** — deep draft/folder/send control
