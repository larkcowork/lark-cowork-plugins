...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The *research engine* stays web/SEO tools — but every durable output is Lark:
> - **Pull internal intel first** before searching the web: prior briefs and win/loss notes in Wiki/Docs (`lark_doc_search` → `lark_doc_fetch`, jq-projected, P3); structured competitor records and deal outcomes in your Lark Base (`lark_base_search` — requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq, P5).
> - **The battlecard lands in Wiki** (P8): `lark_wiki_node_create` → fill via **`lark-doc`**. Sales references it there, not a chat scrollback.
> - **Competitor intel is a Lark Base**, not a static doc (P5): scaffold a "Competitor Tracker" via **`base-deploy`** (fields: Competitor, Tagline, Differentiators, Strengths, Weaknesses, Last Updated, Source URLs); upsert findings with `lark_base_record_upsert`. This makes the brief queryable and the monitoring plan actionable.
> - **Share as an interactive card** (P4): `lark_im_card_send` to the sales/marketing channel — exec summary + "biggest opportunity / biggest threat", with a button linking to the Wiki battlecard. Resolve recipients via `lark_contact_search` (P1).
> - For SEO/keyword competitor data, keep the specialty `~~SEO` tool (Ahrefs/Similarweb) as-is — only the collaboration layer is Lark.
...
**Step 0 — check what you already know (Lark, before the web):** `lark_doc_search` for prior competitive briefs/battlecards in Wiki; `lark_base_search` your Competitor Tracker Base (P5) for existing records and last-updated dates (pass `search_fields` — e.g. the Competitor field; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; no jq); `lark_minutes_search`/`lark_im_search` for recent sales mentions of the competitor (win/loss color). Only then go to the web for net-new intel.
...
- **Community forums**: community forums (e.g. Reddit, Discourse), industry chat groups (e.g. Lark IM communities) — user sentiment
...
- Store the battlecard in **Wiki** (`lark_wiki_node_create`, P8) so it has one durable home; keep the underlying intel in the Competitor Tracker **Base** (`lark_base_record_upsert`, P5) with a "Last Updated" field.
- Review and update quarterly at minimum — offer to set a recurring review **task** (`lark_task_create`, owner resolved via `lark_contact_search`).
- Update immediately after major competitor announcements; capture win/loss feedback from `lark_minutes_search` / sales notes into the Base.
- Track which objection-handling responses are most effective (a column in the Base).

## Output

Present the full competitive brief with clear formatting. Note the date of the research so the user knows the freshness of the data. Then offer to land it natively: publish to **Wiki** (P8), upsert competitor records into the Tracker **Base** (P5), and post an interactive **card** summary to the sales channel (P4).

After the brief, ask:

"Would you like me to:
- Publish a battlecard to Wiki and add these competitors to your Tracker Base?
- Draft messaging that exploits the positioning gaps identified (`/draft-content`)?
- Dive deeper into any specific competitor?
- Set up a competitive monitoring plan? — I can create a recurring review task (`lark_task_create`) and keep the Base as the source of truth."
