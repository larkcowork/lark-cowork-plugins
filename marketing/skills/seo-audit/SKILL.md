...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The audit *engine* stays the specialty `~~SEO` tools (Ahrefs/Similarweb) + web search — but the deliverables are Lark:
> - **Check existing content/coverage in Lark first** before flagging gaps: `lark_doc_search` Wiki/Docs for what you've already published (results under `.data.results[]`); `lark_base_search` your content tracker (requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq, P5).
> - **The action plan lands in Wiki** (P8): `lark_wiki_node_create` → fill via **`lark-doc`**. One durable home, not a chat dump.
> - **Keyword opportunities + content gaps become a Lark Base** (P5): scaffold a "SEO Backlog" via **`base-deploy`** (Keyword, Difficulty, Opportunity, Intent, Content Type, Priority, Status); upsert with `lark_base_record_upsert`. Queryable backlog beats a static table.
> - **Quick wins / content briefs become Lark tasks** (P1/P2): resolve owner via `lark_contact_search` → `lark_task_create(dry_run)` → confirm.
> - **Surface the exec summary as a card** (P4): `lark_im_card_send` — health verdict + top-3 priorities + link to the Wiki plan.
...
**If a specialty ~~product analytics MCP (e.g. Amplitude) is connected:**
- Cross-reference keyword targets with actual organic traffic data to validate which keywords are driving visits and conversions (keep that external tool as-is)

**If no SEO tool is connected:**
- Use web search to research the keyword landscape
- Cross-check against your own published coverage in Lark Wiki/Docs (`lark_doc_search`) and your content tracker Base (`lark_base_search` — requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq) to flag true gaps vs. things you've already covered
...
Offer to land the audit natively: publish the action plan to **Wiki** (P8), load the keyword/gap opportunities into an **SEO Backlog Base** (P5), turn quick wins into **Lark tasks** (P1), and post the exec summary as a **card** (P4).

After presenting the audit, ask:

"Would you like me to:
- Publish the action plan to Wiki and load the opportunities into an SEO Backlog Base?
- Create tasks for the quick wins (assigned to owners)?
- Draft content briefs for the top keyword opportunities (`/draft-content`)?
- Create optimized title tags and meta descriptions for your key pages?
- Build a content calendar based on the gap analysis (`/campaign-plan`)?
- Dive deeper into any specific section, or run this for a different competitor or domain?"
