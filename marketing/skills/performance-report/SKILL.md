...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The number-crunching stays the specialty `~~marketing analytics` / `~~product analytics` tool — but the data of record, the report, and the distribution are Lark:
> - **Read source data from the team's Lark Base / Sheets first** (P5/P3): `lark_base_search` for the campaign/metrics tracker (requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq), `lark_sheets_read` for exported metric tabs (project with `jq` under `.data`). Use the Base data-query endpoint for aggregations rather than pulling raw rows to count client-side.
> - **The report lands in Wiki** (P8): `lark_wiki_node_create` → fill via the **`lark-doc`** skill. This is the canonical monthly/QBR record, not a chat message.
> - **The exec summary goes out as an interactive card** (P4): `lark_im_card_send` — colored `header` (green on-track / red off-track), one `item` row per headline metric with a status pill, an `actions` footer with a button linking to the full Wiki report. Resolve recipients via `lark_contact_search` (P1).
> - **Stakeholder email summary → a Lark Mail draft** (P2): `lark_mail_draft_create` (never auto-send).
> - **Recommendations become tracked work** (P1/P2): each prioritized action → `lark_task_create` (resolve owner first), and append results back to the metrics Base with `lark_base_record_upsert`.
...
3. **Data source** (prefer Lark as the data of record, then specialty analytics):
   - **Lark Base / Sheets first** (P5/P3): `lark_base_search` the campaign/metrics tracker (requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq); `lark_sheets_read` exported metric tabs (jq-projected under `.data`). This is usually where the team's numbers already live.
   - If a specialty `~~marketing analytics` MCP (Supermetrics) is connected, discover available accounts/platforms and pull performance data automatically (keep that tool as-is).
   - If a specialty `~~product analytics` MCP (Amplitude) is connected, pull from it.
   - If none available: ask the user to provide metrics. Prompt with: "Please paste or share your performance data, or point me at a Lark Base/Sheet. I can work with spreadsheets, CSV data, dashboard screenshots described in text, or just the key numbers."
...
Offer to distribute it natively: publish the full report to **Wiki** (P8), post the exec summary as an interactive **card** to the team channel (P4), and create a **Lark Mail draft** of the stakeholder email (P2). Turn each prioritized recommendation into a **Lark task** (P1).

Ask: "Would you like me to:
- Publish this report to Wiki and post the exec summary as a card to your team channel?
- Draft a stakeholder email (Lark Mail draft) with the key takeaways?
- Create tasks for the prioritized recommendations?
- Dive deeper into any specific metric or channel?
- Set up a reusable reporting template (Wiki doc + metrics Base) for next period?"
