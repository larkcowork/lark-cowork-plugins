...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). This is the shared writing-frameworks library behind `/draft-content` and `/email-sequence`; when applied, execution should be Lark-native:
> - **Resolve brand voice from Lark first** (don't assume a generic tone): `lark_doc_search(query="brand voice"/"style guide")` → `lark_doc_fetch` (jq-projected, P3). Reuse facts/proof points from past content in Wiki/Docs and from the team's marketing Base (`lark_base_search` — requires `search_fields`; discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown; narrow with `select_fields`/`limit`, not jq, P5).
> - **Land finished drafts where the org will find them**, not as a chat dump: long-form (blogs, case studies, press releases) → **Wiki/Docs** (`lark_wiki_node_create` + body via the **`lark-doc`** skill, P8); attachments/exports → **Drive** (`lark_drive_upload`).
> - **Route drafts for review safely** (P2): share the draft Doc + a brief review **card** (P4, `lark_im_card_send`) to the reviewer (resolve via `lark_contact_search`, P1). Never auto-publish.
> - Email copy produced here feeds `lark_mail_draft_create` (drafts only — never `confirm_send` without explicit OK).
