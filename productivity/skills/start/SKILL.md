...
> Bootstrap from Lark: tasks via `lark_task_my`; people via `lark_contact_search` (P1); recent
> context via `lark_im_search` / `lark_calendar_agenda` / `lark_minutes_search`; project every read
> under `.data` with `jq` (P3). **Lark Task is the system of record** (P5) — `TASKS.md` is a local mirror.
...
- Lark Task or a Lark Base tracker (via the lark MCP)
- A Lark Doc or notes file
