...
> Decode shorthand/people/projects from Lark sources: `lark_contact_search` (name→open_id, P1),
> `lark_im_search`, `lark_doc_search`, `lark_calendar_agenda`, `lark_minutes_search` — project under
> `.data` with `jq` (P3). The company-tools table maps to Lark (IM/Mail/Base/Wiki). This memory is
> the local **system of record** for who/what/why; back durable context into Lark Wiki when shared (P8).
...
- **todd** → Todd Martinez, Finance lead, prefers Lark IM
...
- Async-first, Lark IM over email
...
|------|----------|---------------|
| Lark IM | Communication | - |
| Lark Task | Engineering tasks | - |
| Lark Base | CRM | "the CRM Base" |
| Lark Wiki | Docs/wiki | - |
...
3. Example: drafting an email to todd about the PSR
   ...
   - memory/people/todd-martinez.md tells you he prefers Lark IM, is direct
