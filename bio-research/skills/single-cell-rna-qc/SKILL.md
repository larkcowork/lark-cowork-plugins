...
> The QC *compute* stays local (scanpy/anndata via the scripts below) — Lark does **not** run the
> analysis. What becomes Lark-native is the **delivery**: share the QC report PNGs and filtered
> `.h5ad` to **Drive**, log the run to the lab **Base** as system-of-record, and announce the result
> as an **interactive card**. See "Share results to Lark" at the end.
...
1. **Upload artifacts to Drive** (P8) — `lark_drive_upload` for the three QC PNGs
   ...
   dir) so they preview inline. Keep the returned file tokens/links for the next two steps.
2. **Log the run to the lab Base** (P5, system-of-record) — `lark_base_record_upsert`
   (`base_token`, `table_id` of the *Runs/Analyses* table) with fields like
   `{skill:"single-cell-rna-qc", input:<basename>, cells_before, cells_after, pct_filtered,
   mt_threshold, output_link:<Drive link>, owner:<open_id>, status:"done"}`. `dry_run: true` first
   ...
   instead, and if the field names are unknown discover them via `lark_api GET
   /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` first (P3). If no lab Base exists yet,
   delegate to `base-deploy`; for record/field details delegate to `lark-base`.
3. **Announce as an interactive card** (P4) — `lark_im_card_send` with a short `header` (e.g.
   ...
   `print_json: true`, then `dry_run: true`, then send. For card grammar delegate to `lark-im`.
4. **Resolve recipients first** (P1) — if posting to a collaborator/DM, `lark_contact_search(name)`
   → `open_id`; `user_ids:"me"` for your own thread. Plain `lark_im_send` only for a one-line ping.
...
- Clustering and cell type annotation (often continued in the **scvi-tools** skill)
