...
> The *methodology* below is unchanged — the change is **where its outputs land and how they get
> shared**. Each of the 9 skills produces a tangible document (Ideation Doc, Risk Matrix, Decision
> Tree Map, Communication Package…). Land each durable artifact in **Wiki** via `lark_wiki_node_create`
> (P8) rather than leaving it as chat text; transient drafts/figures go to **Drive** via
> `lark_drive_upload`. Literature-validation steps stay on the external `~~literature` MCP (PubMed),
> but a recap can be posted as an **interactive card** (`lark_im_card_send`, P4). When a problem
> choice is *committed*, log it as a row in a **Decisions** table of the lab Base
> (`lark_base_record_upsert`, P5). Resolve any advisor/committee member with `lark_contact_search`
> before sharing or scheduling (P1); for a committee/1:1 review, delegate to the `meeting-prep` /
> `one-on-one-prep` skills.
...
3. **Literature Integration** - Use the external `~~literature` MCP (PubMed) at strategic points for validation
4. **Concrete Outputs** - Every skill produces a tangible 1-2 page document → land it in **Wiki** (`lark_wiki_node_create`, P8) so the lab/committee can find it later, not just in chat
...
8. **Decisions are logged** - A committed problem choice / go-no-go becomes a row in the lab Base **Decisions** table (`lark_base_record_upsert`, P5); recap as a card (P4)
