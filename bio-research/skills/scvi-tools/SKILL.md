...
> Model training/inference stays **local** (GPU + the scripts below) — Lark doesn't run scvi-tools.
> The Lark-native part is **collaboration around the run**: trained models, UMAP/embedding plots,
> and DE tables go to **Drive** (P8); each training run is logged to the lab **Base** as
> system-of-record (P5); and integration metrics / completion are surfaced as an **interactive card**
> (P4). Training can be long — notify on completion. See "Share results to Lark" below.
...
## Share results to Lark

Once a model is trained and clustering/DE outputs exist locally, deliver them through the `lark` MCP
(apply P1/P2/P3/P4/P5/P8):

1. **Upload artifacts to Drive** (P8) — `lark_drive_upload` for the UMAP/embedding PNGs, the DE
   table (`de.csv`), and the trained model dir (zip it first). Keep the returned links.
2. **Log the run to the lab Base** (P5) — `lark_base_record_upsert` into the *Runs/Analyses* table:
   `{skill:"scvi-tools", model:<scvi|scanvi|totalvi|…>, input:<basename>, batch_key, n_hvgs,
   n_latent, n_clusters, integration_score, output_link:<Drive>, owner:<open_id>, status}`.
   `dry_run: true` first (P2); read prior runs with `lark_base_search` — it does NOT support `jq` and
   REQUIRES `search_fields` (which field(s) to match); narrow with `select_fields`/`limit` instead, and
   discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if
   unknown (P3). Scaffold via `base-deploy` if no Base exists; record/field ops → delegate to `lark-base`.
3. **Announce as an interactive card** (P4) — `lark_im_card_send`: `header` ("scVI integration:
   12 clusters, kBET 0.86"), `div` rows for metrics, an `image`/link to the UMAP, an `actions`
   button to the Drive report. `print_json: true` → `dry_run: true` → send. Card grammar → `lark-im`.
4. **Notify on completion** — training is long-running; resolve the owner/watcher with
   `lark_contact_search` (P1) and send the completion card to them (or `user_ids:"me"`).
