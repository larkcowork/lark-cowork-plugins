...
> Gather context from the ticket/escalation **Lark Base** (P5), Wiki/Docs known issues, and chat threads (project reads with `jq`, P3). The escalation brief is a **decision** — post it as an **interactive card** (`lark_im_card_send`, P4) into the target team's channel with Acknowledge / Take / Need-info buttons, not a wall of text. When the escalation needs a real human gate (leadership exception, policy override, security sign-off), open a real Lark **approval instance** via `lark_api` (P7) rather than an ad-hoc "reply yes" — delegate exact shapes to **`lark-approval`**. Resolve every target person via `lark_contact_search` (P1). Land the durable brief in **Wiki** (`lark_wiki_node_create`, P8) and create a follow-up **task** (`lark_task_create`) for the owner. For approval-queue/SLA depth, delegate to **`approval-triage`** / **`approval-flow-sla`**.
...
- **~~support platform** (Intercom or ticket Lark Base): `lark_base_search(base_token, table_id, search_fields:["customer"], select_fields:["title","status","priority","customer","timeline"])` for related tickets, timeline, prior troubleshooting. base_search REQUIRES `search_fields` and does NOT support `jq` — if field names are unknown, discover them via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`, then narrow with `select_fields`/`limit`.
- **~~CRM** (HubSpot if its MCP is connected; else the account **Lark Base**): account details, key contacts, prior escalations — `lark_base_search` against the accounts table (pass `search_fields`; no `jq`). Delegate CRM record depth to **`lark-base`** / **`deal-update`**.
- **~~chat** (Lark IM): `lark_im_search(query="<issue / customer>", jq: ".data.messages[]")` for internal discussion and similar reports.
- **~~project tracker** (Lark Task + Base): `lark_task_my(jq: ".data.items[]")` and `lark_base_search` (pass `search_fields`; no `jq`) for the related bug/feature record and its engineering status.
- **~~knowledge base** (Lark Wiki + Docs): `lark_doc_search(query="<symptom>", jq: ".data.results[]")` → `lark_doc_fetch(jq: ".data.content[:4000]")` for known issues / workarounds.
- **Meetings**: if the issue was discussed in a call, `lark_minutes_search(participant_ids="me", query=..., jq: ".data.items[]")` to pull the AI summary + action items (P6) instead of re-deriving.
...
### 7. Deliver the Escalation (Lark-native)

1. **Resolve the target team / owner** via `lark_contact_search` (P1) → `open_id`.
2. **Post the brief as an interactive card** (P4) into the target channel with `lark_im_card_send`: severity-colored `header` (red Critical, orange High), then elements keyed by KIND — `- md:` sections for Impact / What's-been-tried / Repro / What's-needed, and an `- actions:` footer whose buttons each carry a STRING `value` (or a `url`) — Acknowledge / Take / Need-info. Validate `print_json: true` → `dry_run: true` → send. Delegate card grammar to **`lark-im`**.
3. **Open a human gate when required** (P7): leadership exception, policy override, or security sign-off → create a real Lark approval instance via `lark_api POST /open-apis/approval/v4/instances` (form built from the brief). Confirm shapes with **`lark-approval`**.
4. **Land the durable brief in Wiki** (P8): `lark_wiki_node_create(space_id, title:"ESCALATION: …")`, then fill the resulting docx (delegate editing to **`lark-doc`**).
5. **Create a follow-up task** for the owner: `lark_task_create(assignee:<open_id>, summary, due)` with `dry_run: true` first (P2).
6. **Log the escalation** to the ticket/escalation Base for pattern detection: `lark_base_record_upsert` (`dry_run` first).

### 8. Offer Next Steps

After generating the escalation:
- "Want me to post this card in the target team's channel?" → `lark_im_card_send`
- "Should I update the customer with an interim response?" → `/draft-response` → `lark_mail_draft_create`
- "Want me to set a follow-up reminder to check on this?" → `lark_task_create`
- "Should I draft a customer-facing update with the current status?" → `/draft-response`
