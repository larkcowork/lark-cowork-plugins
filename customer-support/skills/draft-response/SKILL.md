...
> **Draft only — never auto-send** (P2). For email, use `lark_mail_draft_create` so a human eyeballs it in the Mail UI; only `lark_mail_send` with `confirm_send: true` after explicit confirmation. For an in-chat reply use `lark_im_send`. Resolve the recipient's `open_id` via `lark_contact_search` first (P1). Pull prior context — thread tone (`lark_api` mail search), account history (ticket/account **Lark Base** via `lark_base_search`, P5 — base_search REQUIRES `search_fields` and does NOT support `jq`; narrow with `select_fields`/`limit`), internal guidance (`lark_im_search`), KB references (`lark_doc_search`) — projecting reads with `jq` where supported (P3). Delegate mail depth to **`lark-mail`** and chat/card depth to **`lark-im`**.
...
Gather relevant background from available sources (project each read with `jq`, P3):

**~~email (Lark Mail):** `lark_api` mail search for prior correspondence, commitments/timelines already shared, and the thread's tone/style.

**~~chat (Lark IM):** `lark_im_search(query="<customer / topic>", jq: ".data.messages[]")` for internal discussion, guidance from product/eng/leadership, and how similar situations were handled.

**~~CRM (HubSpot if connected; else account Lark Base):** `lark_base_search(base_token, table_id, search_fields:["account"], select_fields:["account","plan","contacts","prior_escalations"])` for plan level, key stakeholders, and sensitive history. base_search REQUIRES `search_fields` and does NOT support `jq` — if field names are unknown, discover them via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`.

**~~support platform (Intercom or ticket Lark Base):** `lark_base_search` (pass `search_fields`; no `jq`) for related tickets, resolutions, known issues/workarounds, and SLA/response-time status.

**~~knowledge base (Lark Wiki + Docs):** `lark_doc_search(query=..., jq: ".data.results[]")` → `lark_doc_fetch(jq: ".data.content[:4000]")` for help articles to reference and policy/process docs (don't share unreleased roadmap externally).
...
### 5. Stage the Draft in Lark (never auto-send)

Once the user approves the wording, **stage it where they can review and send** (P2):

- **Email** → `lark_mail_draft_create` (resolve recipient via `lark_contact_search` first, P1). It lands in the Mail UI as a draft; the user clicks send, or you `lark_mail_send` with `confirm_send: true` only on explicit go-ahead.
- **In-chat / support-channel reply** → `lark_im_send` to the resolved `open_id` / chat after confirmation.
- **Follow-up reminder** → `lark_task_create(summary:"Follow up with <customer> if no reply", due:"relative:+3d", dry_run:true)`.

Delegate mail draft/send mechanics to **`lark-mail`** if the thread is non-trivial.
...
- "Want me to draft the internal escalation note as well?" → `/customer-escalation`
- "Should I prepare a follow-up message to send after [X days] if no response?" → `lark_task_create`
