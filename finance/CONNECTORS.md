...
|----------|-------------|-----------|---------------|
| Data warehouse | `~~data warehouse` | Snowflake\*, Databricks\*, BigQuery (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
| Email | `~~email` | Lark Mail (`lark`) | `lark_mail_send`, `lark_mail_draft_create` |
| Office suite | `~~office suite` | Lark Docs/Sheets/Drive (`lark`) | `lark_doc_*`, `lark_sheets_read`, `lark_sheets_append`, `lark_drive_upload` |
| Chat | `~~chat` | Lark IM (`lark`) | `lark_im_send`, `lark_im_search`, `lark_im_card_send` |
| ERP / Accounting | `~~erp` | — (no supported MCP servers yet) (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
| Analytics / BI | `~~analytics` | — (no supported MCP servers yet) (external) | connect this tool's own MCP server; or approximate with Lark Base/Sheets |
