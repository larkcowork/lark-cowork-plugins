...
> [CONNECTORS.md](../../CONNECTORS.md). GL balances, sub-ledger detail, and bank statements come from
> the **ERP / data warehouse / banking** systems (external MCP). Lark owns *tracking, aging, escalation,
> and sign-off*:
> - **Reconciling-items log = a Lark Base** (P5): one row per open item (Account, Description, Amount,
>   Date originated, Age, Category, Status, Owner). Read with `lark_base_search` — it **requires
>   `search_fields`** (e.g. `["Status"]` or `["Account"]`) and does **not** support `jq`; narrow columns
>   with `select_fields` and cap with `limit`. Discover field names first via
>   `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown. Write/age with
>   `lark_base_record_upsert` (`dry_run` first, P2). Scaffold via `base-deploy`.
> - **Aging escalation = tasks + cards**: for items crossing an age/amount threshold, resolve the owner
>   via `lark_contact_search` (P1) → `lark_task_create` (assignee, due); post an **escalation card** (P4)
>   via `lark_im_card_send` with a side button per stale item. For a hard sign-off gate, use a real Lark
>   approval (P7, `lark-approval`).
> - **Sign-off + completed reconciliation** lands in **Wiki/Drive** (P8): `lark_wiki_node_create` for the
>   rec memo, `lark_drive_upload` for the supporting workbook; store the link on the Base row.
> - For the *preparer ≠ reviewer* segregation rule, mirror it in the Base (separate Preparer / Reviewer
>   `open_id` fields) and require the reviewer's approval before Status → Signed-off.
...
Track the age of reconciling items to identify stale items requiring escalation. Pull open items with
`lark_base_search` (`search_fields=["Status"]` to match still-open items — base_search requires the
field(s) to match and has no `jq`; use `select_fields` to return just `Date originated`/`Amount`), then
compute each item's age from `Date originated` yourself. When an item crosses a bucket, update its row
(`lark_base_record_upsert`) and fire the matching action below as a task/card/approval:
...
Define escalation triggers based on your organization's risk tolerance. Each "Escalation" below maps to
a Lark action: a `lark_task_create` assigned to the reviewer (P1 resolve first), an escalation **card**
(P4) to the controller's channel, or a real Lark **approval** (P7) for sign-off gates:
