...
> [CONNECTORS.md](../../CONNECTORS.md). Actuals / budget / forecast come from the **ERP / data
> warehouse / analytics** (external MCP) — or stage them in a **Lark Sheet** (`lark_sheets_read` /
> `lark_sheets_append`). Lark owns *narrative delivery, tracking, and follow-up*:
> - **Variance log = a Lark Base** (P5): one row per material variance (Line item, $ var, % var,
>   Direction, Driver, Outlook, Action, Owner). Read `lark_base_search`, write `lark_base_record_upsert`
>   (`dry_run` first). Scaffold via `base-deploy`. This makes commentary queryable and trendable across
>   periods instead of buried in slides.
> - **The variance bridge / commentary = an interactive card** (P4): post the waterfall + top-driver
>   narratives via `lark_im_card_send` (header, `div` rows per signed driver, `note` net variance), and
>   add an *Investigate* / *Assign* button per item that's flagged `Action: Investigate further`.
> - **Each "Action: Investigate" becomes a task** (P1 → `lark_task_create`, assignee resolved via
>   `lark_contact_search`, due date set). **Land the commentary memo in Wiki** (`lark_wiki_node_create`,
>   P8). Card grammar → delegate to `lark-im`.
...
Good variance narratives should be (store the final narrative on the variance Base row so it's queryable
next period, `lark_base_record_upsert`):
...
When a charting tool is not available, present as a text waterfall (or post it as a card per driver,
P4 — one signed `div` row each; for a real chart, build a dashboard in the variance Base via `base-viz`
/ `base-deploy`):
