...
> [CONNECTORS.md](../../CONNECTORS.md). Trial-balance / actuals / budget data come from the **ERP / data
> warehouse** (external MCP); if none is connected you can stage the numbers in a **Lark Sheet**
> (`lark_sheets_read` to pull, `lark_sheets_append` to build). Lark owns *delivery + review + sign-off*:
> - **The statement itself** lands as a durable doc: build it in a **Lark Doc/Sheet** (`lark_doc_create`
>   / `lark_sheets_append`) and publish the reporting package to **Wiki** (`lark_wiki_node_create`, P8).
>   Upload the source workbook with `lark_drive_upload`.
> - **The leadership summary = an interactive card** (P4): post the key metrics + material-variance flags
>   via `lark_im_card_send` (colored `header`, status-pill `div` rows for each margin, `item` rows for the
>   top material variances with an *Explain* button), not a pasted table. Card grammar → `lark-im`.
> - **Distribute** to the finance leadership channel/people resolved via `lark_contact_search` (P1); for
>   a formal sign-off use a Lark **approval** (P7, `lark-approval`).
> - **Schedule the review meeting** with `lark_calendar_create` (check attendee freebusy via
>   `lark_calendar_freebusy`); pull review action items from `lark_minutes_search` afterward (P6).
> - For deep variance work, hand off to the `/variance-analysis` skill.
...
If ~~erp or ~~data warehouse is connected (external MCP):
...
> Connect ~~erp or ~~data warehouse to pull financial data automatically. You can also paste trial balance data, upload a spreadsheet (`lark_drive_upload`), stage it in a Lark Sheet (`lark_sheets_read` / `lark_sheets_append`), or provide income statement data for analysis.
...
Then land + distribute it the Lark-native way: publish the package to **Wiki** (`lark_wiki_node_create`,
P8), and post the **leadership summary card** (P4, `lark_im_card_send`) — key metrics as status pills,
material variances as `item` rows with an *Explain* button — to the finance leadership channel (people
resolved via `lark_contact_search`, P1). `print_json` → `dry_run` → send (P2).
