...
> [CONNECTORS.md](../../CONNECTORS.md). This is the methodology sibling of the `/sox` command — the same
> Lark layer applies. Transaction populations come from the **ERP / data warehouse** (external MCP);
> Lark holds the *control inventory, workpapers, sample log, and deficiency/remediation tracking*:
> the **control matrix / sample log / deficiency register are Lark Base tables** (P5, scaffold via
> `base-deploy`; read `lark_base_search`, write `lark_base_record_upsert` with `dry_run`); **workpapers
> live in Wiki** (`lark_wiki_node_create`) with evidence in **Drive** (`lark_drive_upload`, P8);
> **testing + remediation are assigned tasks** (`lark_task_create`, owner resolved via
> `lark_contact_search`, P1); **deficiency sign-off is a real Lark approval** (P7, `lark-approval`); and
> the **exception/status summary is an interactive card** (P4, `lark_im_card_send`, grammar → `lark-im`).
...
Organize testing files by control area. In Lark, this folder tree maps to a **Wiki space** (one node per
control area / workpaper, `lark_wiki_node_create`, P8) with evidence files uploaded to **Drive**
(`lark_drive_upload`) and linked from the corresponding control's Base row:
...
For each identified deficiency, create a row in the deficiency-register Base (`lark_base_record_upsert`,
P5), assign the remediation as a task to the owner (`lark_contact_search` → `lark_task_create`, P1), and
gate final sign-off through a Lark **approval** (P7). Track each of:
