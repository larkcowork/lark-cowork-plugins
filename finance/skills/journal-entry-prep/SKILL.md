...
> [CONNECTORS.md](../../CONNECTORS.md). This is the methodology sibling of the `/je` command — the same
> Lark layer applies: the **JE register lives in a Lark Base** (P5, scaffold via `base-deploy`; read
> `lark_base_search`, write `lark_base_record_upsert` with `dry_run`); supporting docs land in **Wiki**
> (`lark_wiki_node_create`) / **Drive** (`lark_drive_upload`, P8); the **approval matrix is a real Lark
> approval instance** (P7, delegate to `lark-approval`), kicked off from a **review card** (P4,
> `lark_im_card_send`); resolve every preparer/approver to an `open_id` via `lark_contact_search` (P1).
...
Every journal entry should have (capture these as **columns of the JE-register Base** row, P5; attach
the calc support / source docs to Wiki or Drive and store the link, P8):
...
Implement each row as a **Lark approval route** (P7): resolve the approver via `lark_contact_search`
(P1) and create the approval instance (`lark-approval` skill / `lark_api`, LARK-RECIPES → Approval).
The Base row's Status field mirrors the approval instance state.
