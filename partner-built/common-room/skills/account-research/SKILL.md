...
> **Lark-native note:** Common Room stays the data source for account research. When findings need to *go somewhere* — outreach drafted, a call prepped, recommended next steps tracked — hand off to the collaboration layer (resolve people with `lark_contact_search`, draft via `lark_mail_draft_create`, surface decisions as a card via `lark_im_card_send`, log to a Base via `lark_base_record_upsert`). See `../../connectors/LARK-PATTERNS.md`, `LARK-RECIPES.md`, `LARK-FUSION.md`. The sibling `compose-outreach` / `call-prep` skills already wire these.
