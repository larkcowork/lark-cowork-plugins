...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [CONNECTORS.md](../../CONNECTORS.md)). The Lark collaboration layer (chat / mail / calendar / docs / tasks / contacts / Base) is available out of the box — never treat it as "not connected." Only money/CRM tools (QuickBooks / PayPal / HubSpot) are external and owner-initiated; a CRM can alternatively live in **Lark Base** (P5). Read stored `## Business context` from session memory to make routing smarter, and resolve any person reference with `lark_contact_search` (P1) if you need to hand a name to a downstream skill.
...
**Connector requirements by command.** The Lark collaboration layer (Lark Mail / Lark Calendar / Lark IM / Lark Drive / Lark Base) is always available — only the external money/CRM tools below are gating. A HubSpot requirement can be satisfied by a **Lark Base** CRM instead (P5).

| Command | Required (external) | Optional |
|---|---|---|
...
| `/call-list` | HubSpot or Lark Base CRM | — (Lark Mail/Calendar built-in) |
| `/run-campaign` | HubSpot, Canva | QuickBooks, PayPal |
| `/sales-brief` | QuickBooks or PayPal | HubSpot |
| `/crm-cleanup` | HubSpot or Lark Base CRM | — |
| `/customer-pulse-check` | PayPal or HubSpot or Lark Base | — |
| `/review-contract` | — (file upload / Lark Mail) | DocuSign |
| `/monday-brief` | — (degrades gracefully; Lark layer built-in) | QuickBooks, PayPal, HubSpot |
| `/friday-brief` | PayPal or HubSpot | — |
| `/quarterly-review` | QuickBooks | PayPal, HubSpot |
| `/handle-complaint` | — (pasted text / Lark Mail built-in) | HubSpot, PayPal |
| `smb-onboard` | — | all |
