...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Sales analysis (QuickBooks / PayPal / Square) is specialty/external — keep it. The **delivery layer is Lark**: present the 30-day brief as an **interactive card** (P4) for owner approval; land the approved brief durably in **Wiki** via `lark_wiki_node_create` (P8) or upload via `lark_drive_upload`. The approved brief then feeds this plugin's `canva-creator` for assets.
