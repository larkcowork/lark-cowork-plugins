...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [CONNECTORS.md](../../CONNECTORS.md)). The collaboration layer is already Lark — chat (`lark_im_*`), mail (`lark_mail_*`), calendar (`lark_calendar_*`), docs/drive (`lark_doc_*` / `lark_drive_upload`), tasks (`lark_task_*`), and contacts (`lark_contact_search`) — so it's connected on day one; only money tools (QuickBooks / PayPal / HubSpot, or build a CRM in **Lark Base** via `base-deploy`, P5) need owner-initiated connection. When proving value with the first recipe, prefer a Lark-native one (e.g. a `morning-brief`-style card) so the owner sees the "Cowork lives inside Lark" magic immediately.
