...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). PayPal/HubSpot order + history stay external. The complaint arrives via **Lark Mail** (search/read with `lark_mail_*` / `lark_api`, project with `jq`) or **Lark IM** (`lark_im_search`); resolve the customer/owner to `open_id` with `lark_contact_search` (P1). The reply is staged with `lark_mail_draft_create` — **never auto-send** (P2). If a Lark Base support log exists, check it for prior themes via `lark_base_search` (P5). For severe escalations needing a manager sign-off, open a real Lark **approval instance** (P7, delegate to **`lark-approval`** / the installed `approval-triage` skill).

Parse arguments:
- `EMAIL_OR_TICKET_ID` (optional) — Lark Mail thread ID, HubSpot ticket ID, or "latest" to pull the most recent unresolved complaint. If omitted, ask the owner to paste the complaint text.

## Step 1 — Load the complaint (ticket-deflector)

Using the `ticket-deflector` skill workflow:

1. If an ID was given: pull the full thread from Lark Mail (`lark_mail_*` / `lark_api`) or HubSpot.
2. If "latest": pull the most recent unresolved HubSpot ticket or Lark Mail thread tagged as complaint/support.
...
1. Check if this complaint matches a known theme (from prior `/customer-pulse-check` runs, a Lark Base support log via `lark_base_search`, or similar complaints in HubSpot).
...
If Lark Mail (`lark_mail_*`) and HubSpot are both unreachable, ask the owner to paste the complaint text — the skill works with manual input. If PayPal is missing, skip transaction lookup and note "PayPal not connected — order status unavailable, working from complaint text only."
