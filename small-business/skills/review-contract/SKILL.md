...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Legal analysis is specialty; DocuSign stays external. **Collaboration is Lark**: source the file from Lark Mail / Lark Drive; upload the marked-up DOCX/PDF with `lark_drive_upload`. Delegates the analysis to this plugin's `contract-review`.
...
Offer to export this as a marked-up docx or PDF; for Files, upload via `lark_drive_upload`.
