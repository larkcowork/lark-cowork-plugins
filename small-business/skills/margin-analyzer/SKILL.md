...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). QuickBooks (COGS) + PayPal/Square (revenue) stay external — this is a specialty finance analysis. Only the **delivery layer is Lark**: surface the unit-economics summary as an **interactive card** (P4) and upload any exported workbook with `lark_drive_upload`. The skill stays read-only (no price recommendation, no writes).
