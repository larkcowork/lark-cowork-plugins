...
## Gotcha: Lark Mail auth failure mid-pulse

**Why it matters:** Lark Mail auth is intermittently flaky in workshop environments. Surfacing a raw auth error in the middle of the pulse looks broken and breaks the owner's trust in the skill.

### ✗ Bad

```
Claude: [Lark Mail auth error mid-composition]
        → "Error: Lark Mail authentication failed. Please re-authenticate."
...
```
Claude: [Lark Mail auth error]
        → continues pulse without Watch List section
        → appendix: "Lark Mail — auth error; Watch List unavailable this run"
...
## Gotcha: Lark IM write requires explicit confirmation

**Why it matters:** Lark IM write is not tested in the standard validation path and posts to channels other people can see. Auto-posting without confirmation could embarrass the owner or spam a team.
...
Owner never asked for a Lark IM post; now the whole team has the financial data.

### ✓ Good

```
Claude: [at end of pulse]
        "Want me to post this to Lark IM? If so, which channel?"
```

Lark IM write only happens with a specific "yes + channel name" from the owner. Never assume.
