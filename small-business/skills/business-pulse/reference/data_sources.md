...
**Lark Mail fallback**: if the Lark Mail call errors (auth flaky — this is a known issue), skip Watch List silently and add "Lark Mail unavailable" to the appendix. Do not surface the error in the pulse body.

## Internal Signals (Lark IM / Teams)

| Metric | Tool | Notes |
|---|---|---|
| Urgent threads | Lark IM search (if connected) | Threads with @mentions or urgency signals in owner-relevant channels |
| Action items | Lark IM search | Messages directed at the owner or tagged for follow-up |
...
| Urgent Lark Mail threads | Lark Mail | `is:important` or escalation keywords |
