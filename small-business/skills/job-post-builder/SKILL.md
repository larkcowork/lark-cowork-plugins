...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The e-signature route (DocuSign) and web research stay external. The **collaboration layer is Lark**: search prior JDs/templates in **Lark Drive** (`lark_doc_search`); upload the three packet DOCXs with `lark_drive_upload`, or author them natively in **Wiki/Docs** via `lark_wiki_node_create` / `lark_doc_create` (P8, delegate editing to **`lark-doc`**); resolve the candidate/owner to `open_id` with `lark_contact_search` (P1). The **DocuSign fallback email is Lark Mail** — stage it with `lark_mail_draft_create` (never auto-send, P2).
...
2. **Research comparable posts** — Search Lark Drive / Desktop for existing JDs
   and templates (`lark_doc_search`); run web search for 3–5 live postings for
   this role. Sources: Lark Drive (`lark`), web search.
...
- **Never send the Lark Mail fallback email without approval.** If the DocuSign browser
...
- **Existing JD or template?** — ask if there's a prior version in Lark Drive or on their Desktop to use as a starting point
...
Search Lark Drive and Desktop for prior JDs, offer letter templates, or
interview guides the user may already have. Use `lark_doc_search` with terms like
the role title, "job description", "JD", "offer letter", "interview". If found,
read them (`lark_doc_fetch`, project with `jq`, P3) and use them as the baseline —
preserving any existing language, structure, or requirements the user has established.
...
Save as `[Role]-Job-Post.docx` using the docx skill, then upload via
`lark_drive_upload` so it's shareable from Lark (or author it natively in Wiki
with `lark_wiki_node_create` + `lark-doc`, P8).
...
Save as `[Role]-Interview-Guide.docx` using the docx skill, then upload via `lark_drive_upload`.
...
Save as `[Role]-Offer-Letter.docx` using the docx skill, then upload via `lark_drive_upload`.
...
fall back to the Lark Mail draft approach: stage an email with `lark_mail_draft_create`
(resolve the candidate via `lark_contact_search`, P1) with the offer letter attached and
a note to upload it to DocuSign manually. Show the draft to the user — never auto-send (P2).
