...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Sales data (PayPal / QuickBooks) and HubSpot campaign cross-ref stay external. **Delivery is Lark**: present the 2-week brief + calendar as an **interactive card** (P4), and land it durably in **Wiki** (`lark_wiki_node_create`, P8) or upload via `lark_drive_upload`. Delegates analysis to this plugin's `content-strategy`.
