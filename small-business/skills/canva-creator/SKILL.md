...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Canva (social design) and HubSpot (social staging) stay on their external connectors — they have no Lark equivalent. The **collaboration layer is Lark**: for the **email path**, stage the drafted email with `lark_mail_draft_create` instead of only surfacing inline (never auto-send, P2); save exported assets/CSVs with `lark_drive_upload`; present each approval stage as an **interactive card** (P4). Resolve any recipient to `open_id` with `lark_contact_search` (P1).
...
4. **Surface email content for handoff.** For each email row, present the
   approved subject + preheader + body grouped by send date, then offer to
   stage each as a **Lark Mail draft** with `lark_mail_draft_create`
   (`dry_run: true` first, never auto-send, P2) so the owner reviews and
   sends from the Lark Mail UI. If the owner prefers another tool (HubSpot
   Marketing Email, Mailchimp), surface the copy inline instead.
