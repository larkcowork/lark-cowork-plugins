...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md)). PayPal/HubSpot/Intercom signals stay on their external connectors. The **Lark layer**: pull email threads via `lark_mail_*` (project with `jq`, P3) and internal chatter via `lark_im_search`; if the owner keeps a ticket/feedback log in **Lark Base**, read it with `lark_base_search` (P5). This skill is read-only — but you may offer to land the themes report durably in **Wiki** (`lark_wiki_node_create`, P8) or surface it as an **interactive digest card** (`lark_im_card_send`, P4) instead of plain text.
...
4. **Pull Lark Mail threads.** Search via `lark_mail_*` / `lark_api` mail search for threads in the window containing: `refund cancel unhappy issue problem disappointed frustrated broken late slow wrong missing`. Extract subject lines and 1–2 sentence excerpts per thread (project with `jq`, P3). Also sweep `lark_im_search` for the same keywords in support/ops chats.
...
7. **Extract themes.** Group all evidence into 3–5 recurring themes. Each theme must include:
   ...
   - 2–3 verbatim quotes with source tags: `[PayPal]`, `[HubSpot]`, `[Lark Mail]`, `[Intercom]`, or `[Review]`
...
9. **Deliver the report.** Structure the output with these sections in order:
   ...
   - **Sources pulled** — Bullet list with signal counts per source (PayPal
     disputes, HubSpot tickets, Lark Mail threads, Intercom conversations, pasted
...
- [reference/gotchas.md](reference/gotchas.md) — PayPal rate limits, HubSpot empty state, verbatim quote requirement, Lark Mail keyword drift
