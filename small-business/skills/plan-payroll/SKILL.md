...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Cash/AR data stays on QuickBooks / PayPal / Stripe / Square (external). **Delivery is Lark**: upload the forecast XLSX with `lark_drive_upload`; stage chase reminders via `lark_mail_draft_create` (never auto-send, P2); surface the forecast verdict + reminder batch as an **interactive card** (P4). Chains this plugin's `cash-flow-snapshot` and `invoice-chase`.
...
4. PayPal-issued invoices queue as PayPal-send drafts; non-PayPal invoices queue as Lark Mail drafts (`lark_mail_draft_create`).
