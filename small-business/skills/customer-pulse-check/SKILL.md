...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). PayPal/HubSpot stay external; email + chat signals come from Lark Mail and `lark_im_search`, and a Lark Base ticket log (if used) via `lark_base_search` (P5). Response templates are staged with `lark_mail_draft_create` — **never auto-send** (P2). Surface the top-3-fixable summary as an **interactive card** (P4). Delegates response drafting to this plugin's `ticket-deflector`.
...
Present the summary table (as an interactive card, P4), then each response template. Ask the owner which templates they'd like to send, then on approval stage each via `lark_mail_draft_create` — never auto-send (P2).
