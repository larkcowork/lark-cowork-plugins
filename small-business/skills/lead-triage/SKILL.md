...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)).
> Lead data comes from **HubSpot** (external) or a **Lark Base** CRM (`lark_base_search` — this tool does **not** support `jq`; it **requires** `search_fields` naming the field(s) to match, so discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown and narrow output with `select_fields`/`limit`, P5). The **collaboration layer is Lark**: match a lead's last outbound thread via Lark Mail; draft follow-ups with `lark_mail_draft_create` (never auto-send, P2); check open windows with `lark_calendar_agenda` / `lark_calendar_freebusy` and create holds with `lark_calendar_create` only after confirmation; resolve every lead/owner name to `open_id` with `lark_contact_search` (P1). Surface the ranked list as an **interactive card** with a per-lead action button (P4). For ranking depth, the installed **`task-prioritizer`** and `pipeline-review` skills are reusable.

## Quick start

Pull inbound leads (HubSpot or Lark Base), score them, and surface a ranked call list with talking points. Drafts follow-ups and proposes calendar slots — never sends or books without owner approval.
...
5. **Offer follow-up drafts.** Ask: *"Draft follow-ups for any of these?"* If yes, write one email per selected lead, matching the tone of their last outbound thread in Lark Mail, and stage each with `lark_mail_draft_create`. Show draft; do not send (P2).

6. **Offer calendar slots.** Ask: *"Propose call slots for any of these?"* If yes, check `lark_calendar_agenda` / `lark_calendar_freebusy` for open 30-minute windows in the next two business days (avoid slots with existing events ±15 min). Propose two options per lead. Do not create events — the owner books (or, on confirmation, create the hold with `lark_calendar_create`).
