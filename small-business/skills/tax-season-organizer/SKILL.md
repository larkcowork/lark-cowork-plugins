...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Tax math + QuickBooks / PayPal / Stripe pulls are specialty/external — keep as-is. Only the **delivery layer is Lark**: upload the accountant handoff document via `lark_drive_upload`; optionally draft the cover email to the accountant with `lark_mail_draft_create` (never auto-send, P2, resolve them via `lark_contact_search`, P1); land a durable copy in a tax Wiki space with `lark_wiki_node_create` (P8). The skill files nothing and gives no tax advice.
