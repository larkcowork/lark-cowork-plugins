...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). AR/AP and settlement data stay on QuickBooks / PayPal / Stripe / Square (external). The **delivery layer is Lark**: upload the finished XLSX with `lark_drive_upload`, surface the chat summary + risk flags as an **interactive card** (`lark_im_card_send`, P4), and — if the owner keeps a finance log in Lark Base — write the forecast snapshot via `lark_base_record_upsert` (P5). Resolve any recipient to `open_id` with `lark_contact_search` (P1).
...
Save as `cash-flow-snapshot-[YYYY-MM-DD].xlsx`, then upload to Lark Drive with `lark_drive_upload` so the owner can open it in Lark.
