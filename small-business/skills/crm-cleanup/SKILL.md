...
Run a CRM hygiene pass using the `crm-maintenance` skill cleanup workflow. Act immediately — the user typed /crm-cleanup, so skip the intent-detection step.

> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The CRM may be **HubSpot** (external) or a **Lark Base** (P5). For the Base backend: scan with `lark_base_search` (this tool does **not** support `jq`; it **requires** `search_fields` naming the field(s) to match — discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown, and narrow output with `select_fields`/`limit`), apply approved fixes with `lark_base_record_upsert` (`dry_run` first, P2); delegate record/field specifics to **`lark-base`**. Resolve any owner/contact name to `open_id` via `lark_contact_search` (P1). Surface the stale-deal / duplicate / missing-field findings as an **interactive card** with a per-row action button (P4) so the owner triages inline.
