...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Cash data stays on QuickBooks / PayPal (external). **Delivery is Lark**: surface the heads-up + two watches as an **interactive card** (P4); for flagged overdue invoices, stage chase emails with `lark_mail_draft_create` (never auto-send, P2) — delegates to this plugin's `invoice-chase`. Resolve recipients to `open_id` via `lark_contact_search` (P1).
...
Present the formatted brief (as an interactive card, P4) and offer to stage chase emails via `lark_mail_draft_create` for any flagged overdue invoices (never auto-send, P2).
