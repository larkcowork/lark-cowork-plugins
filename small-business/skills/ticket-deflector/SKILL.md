...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). PayPal (order/refund), HubSpot, Intercom, Square stay on their external connectors. The **Lark layer**: read the forwarded thread via Lark Mail (`lark_mail_*` / `lark_api`); resolve the customer/owner to `open_id` with `lark_contact_search` (P1); stage the reply with `lark_mail_draft_create` (or, on approval, send with `lark_mail_send confirm_send: true`) — **never auto-send** (P2). Log the interaction to a **Lark Base** support log via `lark_base_record_upsert` (`dry_run` first, P5) when the owner keeps one. A **refund is a money gate** — drive it through a real Lark **approval instance** (P7, delegate to **`lark-approval`**) rather than an ad-hoc "reply yes."
...
6. **Approval gate — refund issuance.** If a refund is warranted, prefer a real Lark **approval instance** (P7) over an ad-hoc yes/no — create it via `lark_api` (`POST /open-apis/approval/v4/instances`, see LARK-RECIPES; delegate to **`lark-approval`** for exact form shapes) with the customer, amount, and transaction ID in the form, routed to the owner/approver (resolve `open_id` with `lark_contact_search`). The approval engine tracks state and notifies. For a lightweight one-off, a confirmation card (P4) is acceptable:

   > *"Issue refund of $[amount] to [customer name] ([email]) for transaction [ID]? Reply Y to proceed."*

   Wait for explicit confirmation. If the owner's reply is anything other than a clear yes, stop and ask what they'd like to do instead. Only execute the PayPal refund after the approval/confirmation resolves.

7. **Send or stage the reply.** After draft approval, ask the owner: send via Lark Mail now (`lark_mail_send` with `confirm_send: true`), or keep it as a draft (`lark_mail_draft_create`)? Execute their choice (`dry_run: true` first, P2). Then log the interaction — as a note on the HubSpot contact timeline, or as a row in the Lark Base support log via `lark_base_record_upsert` (P5).
