...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). QuickBooks / PayPal / HubSpot financials stay external. **Delivery is Lark**: save the QBR via `lark_drive_upload`; land the durable narrative in **Wiki** with `lark_wiki_node_create` (P8); surface the headline + top opportunities/risks as an **interactive digest card** (P4). Customer-health data can also come from a Lark Base CRM (`lark_base_search`, P5). For a board-ready deck, delegate slide authoring to the installed **`lark-slides`** skill.

Parse arguments:
- `--quarter` (default: previous calendar quarter) — format `YYYY-QN` (e.g., `2026-Q1`)
- `--save-to` (default: `files`) — `files` (Lark Drive via `lark_drive_upload`), `desktop`, or `both`
...
2. Save to `--save-to` location; for `files`, upload via `lark_drive_upload`. Optionally land the narrative in a QBR Wiki space with `lark_wiki_node_create` (P8).
