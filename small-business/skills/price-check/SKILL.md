...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Cost/revenue data stays on QuickBooks / PayPal (external). **Delivery is Lark**: present the margin + scenario tables as an **interactive card** (P4); if the owner wants the customer messaging brief sent, stage it via `lark_mail_draft_create` (never auto-send, P2); save any exported workbook with `lark_drive_upload`. Delegates the unit-economics analysis to this plugin's `margin-analyzer`.
