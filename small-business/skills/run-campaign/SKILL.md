...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). Sales analysis (QuickBooks/PayPal), Canva assets, and the HubSpot bulk send stay on their external connectors. The **Lark layer**: present each approval gate as an **interactive card** (P4); the high-priority call list books holds via `lark_calendar_create` (check `lark_calendar_freebusy` first) and stages call follow-ups with `lark_mail_draft_create` (P2); resolve contacts to `open_id` via `lark_contact_search` (P1). Chains this plugin's `content-strategy` → `canva-creator` → `lead-triage`.
...
4. Block calendar time for the call list via `lark_calendar_create` (after confirmation; check `lark_calendar_freebusy` for open slots).
