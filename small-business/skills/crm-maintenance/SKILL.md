...
> The CRM can live in **Lark Base** (P5) — Lark has no separate CRM product, a Base *is* the database. Two backends:
> - **HubSpot** (external MCP) — keep the steps below as written.
> - **Lark Base CRM** — read records with `lark_base_search` (this tool does **not** support `jq`; it **requires** `search_fields` naming which field(s) to match — narrow with `select_fields`/`limit` instead. If you don't know the field names, discover them first via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`), write with `lark_base_record_upsert` (create-or-update, `dry_run` first per P2). To scaffold a CRM Base (Contacts / Deals / Activity tables), don't hand-roll schema — invoke the **`base-deploy`** skill; for record/field ops delegate to **`lark-base`**.
> Context inputs are Lark either way: read email threads via Lark Mail, meetings via `lark_calendar_agenda`, and **always resolve people to `open_id` with `lark_contact_search` before writing** (P1). For deeper post-call CRM updates from a meeting, the installed **`deal-update`** skill (Minutes → Base record → follow-up draft) is a near-drop-in.

## Quick start

Pull context from the referenced email or calendar event, resolve the right contact and deal (HubSpot or Lark Base), log the activity, and surface what changed. For a deal cleanup, audit the deal against recent email/calendar activity and propose updates — never apply them without approval.
...
2. **Gather context.**
   - Email path: read the thread via Lark Mail (subject, participants, last 1–3 messages; project with `jq`). Identify the primary external contact.
   - Call path: read the calendar event via `lark_calendar_agenda` (title, attendees, time, description). If no event was specified, use the most recent completed meeting in the last 24 hours and confirm with the user before proceeding. If a Minutes recording exists, prefer its AI artifacts via `lark_minutes_search` (P6) over re-deriving notes.
...
3. **Resolve the contact and deal.** For email/call paths:
   - **HubSpot backend:** search HubSpot contacts by email. **Lark Base backend:** `lark_base_search` the Contacts table by email — pass `search_fields` naming the email field (discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown); `lark_base_search` does not support `jq`, so narrow output with `select_fields`/`limit`. If a contact is missing, create it from email signature or calendar invite data — announce creation in chat before writing (`lark_base_record_upsert` with `dry_run` first for the Base backend).
   ...
   - HubSpot field names, activity types, and association rules: read [reference/hubspot-fields.md](reference/hubspot-fields.md) before writing. For the Lark Base backend, delegate field/table specifics to the **`lark-base`** skill.
...
4. **Execute the action.** (Lark Base backend: every write is `lark_base_record_upsert` into the Activity table, `dry_run: true` first to preview the recipient + payload, then commit on confirm, P2.)
