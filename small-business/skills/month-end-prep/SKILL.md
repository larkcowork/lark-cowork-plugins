...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). QuickBooks + PayPal/Square/Stripe stay external. The **collaboration layer is Lark**: present the Step-6 sign-off summary and per-flag triage as an **interactive card** (P4); save the close packet via `lark_drive_upload`; land the durable P&L narrative in **Wiki** with `lark_wiki_node_create` (P8). The Desktop receipts scan stays local. Resolve any recipient to `open_id` with `lark_contact_search` (P1).
...
Save both to the Desktop (or a path the user specifies), then upload to Lark Drive via `lark_drive_upload` so the packet is shareable from Lark. Confirm the file locations.
