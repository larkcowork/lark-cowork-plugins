...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). Tax math + QuickBooks/PayPal/Stripe data are specialty/external — keep as-is. **Delivery is Lark**: upload the accountant handoff packet via `lark_drive_upload`, and optionally email the accountant a draft (`lark_mail_draft_create`, never auto-send, P2) after resolving them with `lark_contact_search` (P1). Delegates the calculation to this plugin's `tax-season-organizer`.
