...
description: >
  ...
  staff. Reads contracts from local files, Lark Mail attachments, or DocuSign
...
> **Lark-native execution** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The legal analysis is specialty work — keep it. DocuSign stays external. The **collaboration layer is Lark**: pull contract attachments from **Lark Mail** (`lark_mail_*` / `lark_api` mail search, then download via the mail/drive recipe); upload the redlined DOCX with `lark_drive_upload`; resolve the counterparty/owner to `open_id` with `lark_contact_search` (P1). For non-trivial doc editing, delegate to the installed **`lark-doc`** / `lark-drive` skills.
...
1. **Get the contract** — Pull from one of three sources, in order of preference:
   - **Lark Mail**: search recent emails for contract attachments via `lark_mail_*` / `lark_api` mail search, then download the attachment (see `reference/gmail-fetch.md`; for the download step delegate to **`lark-drive`** / `lark-mail`)
...
5. **Export redline DOCX** — After presenting the summary, offer to export a redlined DOCX with the suggested changes marked up, then upload it via `lark_drive_upload`. Use the `docx` skill to generate a Word document that:
...
- `reference/gmail-fetch.md` — finding contract attachments in Lark Mail
