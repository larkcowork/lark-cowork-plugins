...
> External research (web, Similarweb competitive-intelligence) stays on its own tools — there is no
> Lark equivalent for market data. But the **collaboration layer is Lark**: search prior competitive
> work and win/loss reports in Docs/Wiki (`lark_doc_search`/`lark_doc_fetch`, jq-projected, P3) and
> sales/product chatter via `lark_im_search`; pull deal/win-loss records from a **CRM/feedback Base**
> (`lark_base_search` — REQUIRES `search_fields`, no `jq`, narrow with `select_fields`/`limit`, P5).
> Land the finished brief and battle cards in **Wiki**
> (`lark_wiki_node_create`, P8); post the strategic-implications summary as an **interactive card**
> (`lark_im_card_send`, P4). A live competitor tracker can be a **Lark Base** — scaffold via
> **`base-deploy`**.
...
**Via web search / competitive-intelligence tools** (external — Similarweb etc. keep their own MCP):
...
**From Lark (collaboration layer, project reads with `jq` where supported, P3):**
- Knowledge base — `lark_doc_search` for existing competitive analysis docs, win/loss reports, and
  sales battle cards; `lark_doc_fetch` the relevant ones.
- Chat — `lark_im_search` sales/product channels for competitive mentions and recent deal feedback
  involving competitors.
- CRM/deal Base (P5) — `lark_base_search` for closed-won/lost records tagged with the competitor, to
  ground win/loss analysis in real data. `lark_base_search` REQUIRES `search_fields` (e.g. the
  competitor-tag field; discover field names via
  `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown) and does NOT
  support `jq` — narrow with `select_fields`/`limit`.
...
### 4. Follow Up (Lark-native)

After generating the brief:
- Ask if the user wants to dive deeper on any section.
- Land the brief in **Wiki** via `lark_wiki_node_create` (P8) so the org finds it later; post the
  strategic-implications summary as an **interactive card** via `lark_im_card_send` to the product
  channel (P4).
- Offer a one-page exec summary — delegate the broadcast to the **stakeholder-update** skill.
- Offer to create sales battle cards / a "how to win against [competitor]" guide as Wiki docs, and
  to store them as rows in a competitor-tracker **Base** (`lark_base_record_upsert`, `dry_run`
  first, P2).
- Offer a monitoring plan: set recurring review tasks via `lark_task_create`, or scaffold a live
  competitor-tracker Base via **`base-deploy`** (P5).
