...
> Pull source material from Lark: roadmap/sprint state from a **Base** (`lark_base_search` —
> REQUIRES `search_fields`, no `jq`, narrow with `select_fields`/`limit`, P5),
> recent decisions from `lark_im_search`, meeting outcomes from `lark_minutes_search` (P6 — use the
> AI summary, don't re-derive), prior docs from `lark_doc_search`/`lark_doc_fetch` (project with
> `jq`, P3). Then deliver to the right surface: exec/board → **interactive card**
> (`lark_im_card_send` with G/Y/R `header`, P4); cross-functional/customer → **email draft**
> (`lark_mail_draft_create` — never auto-send, P2); durable record → Wiki (`lark_wiki_node_create`,
> P8). Resolve recipients with `lark_contact_search` (P1). For the metrics section, delegate to the
...
### 3. Pull Context from Lark (project reads with `jq` where supported, P3)

- **Project tracker (Base + Task, P5):** `lark_base_search` the roadmap/sprint table for item
  status, milestones, completed-since-last-update, and at-risk/blocked items. Note: `lark_base_search`
  does NOT support `jq` and REQUIRES `search_fields` (discover field names via
  `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown); narrow with
  `select_fields`/`limit` instead.
  `lark_task_my` / the task-list endpoint (LARK-RECIPES) for delivery progress.
- **Chat:** `lark_im_search` for recent team discussions, decisions made async, and blockers raised
  in channels.
- **Meeting transcription (Minutes, P6):** `lark_minutes_search(participant_ids="me", query=...)` →
  pull the AI summary, decisions, and action items directly instead of re-summarizing.
- **Knowledge base (Docs/Wiki):** `lark_doc_search` for recent meeting notes, decision documents,
  design reviews; `lark_doc_fetch` (with `jq` projection) for the ones you'll cite.

If the user prefers to supply context manually, ask for: what was accomplished since last update,
current blockers/risks, key decisions made or needed, and what is coming next.
...
### 5. Review and Deliver (Lark-native)

After generating the update:
- Ask if the user wants to adjust tone, detail level, or emphasis.
- Deliver to the surface that fits the audience (resolve recipients via `lark_contact_search`, P1):
  - **Exec / board:** post an **interactive card** with `lark_im_card_send` — a colored G/Y/R
    `header`, `div` rows for progress/risks, an `actions` footer for "decisions needed". Validate
    with `print_json: true`, then `dry_run: true`, then send (P4).
  - **Cross-functional / customer:** draft an **email** with `lark_mail_draft_create` so the user
    eyeballs it in the Mail UI before sending — never auto-send (P2).
  - **Engineering / async channel:** `lark_im_send` to the team channel, or a card if it carries
    decisions/asks.
  - **Durable record:** land the update in Wiki via `lark_wiki_node_create` (P8).
