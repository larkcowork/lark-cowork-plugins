...
> **Lark-native note** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md),
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The session is pure conversation — no tool
> calls mid-flow. Two seams touch Lark: (1) optionally **ground** the conversation in context you
> read with `jq` projection where supported (P3) — prior research via `lark_doc_search`, what's been
> tried via the project Base (`lark_base_search` — REQUIRES `search_fields`, no `jq`, narrow with
> `select_fields`/`limit`, P5), recent discussion via `lark_im_search`; (2) at **Capture**,
> land durable notes in Wiki (`lark_wiki_node_create`, P8) and hand off to the owning skills
> (`write-spec`, `synthesize-research`, `competitive-brief`) rather than producing the deliverable
> here.
...
**Tips:**
- The job is stable even when solutions change. People have been "hiring" solutions to share updates with colleagues for decades — memos, email, Lark IM, shared docs.
...
Document what matters. A brainstorm with no capture is a brainstorm that never happened. Land the
capture as a durable Wiki page via `lark_wiki_node_create` (P8) — not a throwaway chat message:

- Key ideas and why they are interesting
- Assumptions to test
- Questions to research
- Suggested next steps (research, prototype, talk to users, write a one-pager) — offer to create
  these as tasks via `lark_task_create` (resolve owners with `lark_contact_search`, P1)
