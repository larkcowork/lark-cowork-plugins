...
> Pull qualitative inputs from Lark: interview recordings via `lark_minutes_search` (P6 — use AI
> summaries/transcripts), research docs via `lark_doc_search`/`lark_doc_fetch`, raw feedback from a
> **feedback Lark Base** via `lark_base_search` (the system-of-record for tickets/feature requests,
> P5; `lark_base_search` does NOT support `jq` and REQUIRES `search_fields` — narrow with
> `select_fields`/`limit`). Project the other reads with `jq` (P3 — interview transcripts are large).
> Land the finished
> synthesis as a durable Wiki page via `lark_wiki_node_create` (P8) and post the top findings as an
> **interactive card** (`lark_im_card_send`, P4). Specialty product-analytics tools (Amplitude,
> Pendo) and the feedback product (Intercom) keep their own MCP servers — use them as-is for
> quantitative/behavioral data; only the collaboration layer is Lark.
...
Accept research from any combination of (project Lark reads with `jq` where supported — note
`lark_base_search` is the exception: no `jq`, requires `search_fields`, P3):
- **Pasted text / uploaded files**: Interview notes, transcripts, survey responses, feedback
  (upload supporting files to Drive via `lark_drive_upload` if they should be retained).
- **Knowledge base (Docs/Wiki)**: `lark_doc_search` for research documents, interview notes, survey
  results; `lark_doc_fetch` (jq-projected) for the ones you'll cite.
- **User feedback (Base, P5)**: `lark_base_search` a feedback/support tracker Base for recent
  tickets, feature requests, bug reports. Note: `lark_base_search` does NOT support `jq` and REQUIRES
  `search_fields` (discover field names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`
  if unknown); narrow with `select_fields`/`limit` instead of `jq`. *(If feedback lives in Intercom, use its own MCP server.)*
- **Meeting transcription (Minutes, P6)**: `lark_minutes_search` for interview recordings → pull AI
  summaries and transcripts directly instead of re-deriving.
- **Product analytics**: Amplitude/Pendo via their own MCP server for usage/funnel/behavioral data
  (no Lark equivalent — use the specialty tool as-is). Approximate with `lark_sheets_read` only if
  the data has been exported into a Lark Sheet.
...
### 5. Review and Extend (Lark-native)

After generating the synthesis:
- Ask if any findings need more detail or different framing.
- Land the synthesis as a durable Wiki page via `lark_wiki_node_create` (P8); post the top 5-8
  findings as an **interactive card** to the product channel via `lark_im_card_send` (P4).
- Offer to generate specific artifacts: persona documents (`lark_doc_create`), opportunity maps,
  research presentations (delegate to the `lark-slides` skill).
- Offer to promote prioritized opportunities into the roadmap Base via `lark_base_record_upsert`
  (`dry_run` first, P2) and feed them to the **roadmap-update** skill.
- Offer to capture follow-up research items as tasks via `lark_task_create` (resolve owner with
