...
> Raw product-analytics numbers come from a specialty tool (Amplitude/Pendo on its own MCP server) —
> no Lark equivalent, use it as-is. But Lark owns: **targets/OKRs** (`lark_okr_cycle_list` →
> compare metrics against KRs; delegate to the **`lark-okr`** skill for deep reads), **metrics that
> already live in a Lark Sheet/Base** (`lark_sheets_read` jq-projected; `lark_base_search` does NOT
> support `jq` and REQUIRES `search_fields` — narrow with `select_fields`/`limit`, P3), and
> the **delivery surface**. Deliver the scorecard as an **interactive card** with G/Y/R status pills
> (`lark_im_card_send`, P4); land the durable review in Wiki (`lark_wiki_node_create`, P8). For an
> ongoing KPI dashboard, build a **Lark Base** dashboard via **`base-deploy`** rather than describing
> one (P5). For the OKR narrative, you may also reuse the **weekly-review** skill.
...
- **Product analytics (external):** Amplitude/Pendo via their own MCP server — pull key metrics for
  the period, comparison data (previous period, YoY, targets), and segment breakdowns. No Lark
  equivalent.
- **Metrics already in Lark:** if numbers are tracked in a Lark Sheet, read with `lark_sheets_read`
  (project with `jq`, P3). If tracked in a Base, read with `lark_base_search` — it does NOT support
  `jq` and REQUIRES `search_fields`; narrow with `select_fields`/`limit` instead (discover field
  names via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown).
- **Targets / OKRs:** `lark_okr_cycle_list` to fetch the active cycle's objectives and KR targets to
  compare against (delegate to **`lark-okr`** for objective/KR detail).

If no source is available, ask the user to provide: the metrics and their values (paste a table,
screenshot, or describe), comparison data, and context on recent changes (launches, incidents,
seasonality).
...
- Deliver the scorecard as an **interactive card** via `lark_im_card_send` — colored `header`,
  `div` rows with G/Y/R status pills per metric, `note` footer. Validate with `print_json: true`,
  then `dry_run: true`, then send (P4). Land the durable review in Wiki (`lark_wiki_node_create`, P8).
- Ask if any metric needs deeper investigation.
- Offer an **ongoing dashboard**: build a real Lark Base KPI dashboard by delegating to
  **`base-deploy`** (P5), not just a spec.
- Offer to capture investigations / experiment proposals as assigned tasks via `lark_task_create`
  (resolve owner via `lark_contact_search`, P1).
- Offer to set up a recurring metrics-review template doc in Wiki.
