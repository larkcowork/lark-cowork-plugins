...
> The roadmap lives in a **Lark Base** as system-of-record (P5): read with `lark_base_search`
> (it does NOT support `jq` — it REQUIRES `search_fields`; narrow with `select_fields`/`limit`
> instead), write status/priority/date changes with `lark_base_record_upsert`
> (`dry_run` first, P2). Resolve owners to `open_id` with `lark_contact_search` (P1). Roadmap items
> that are concrete deliverables also become `lark_task_create` tasks. Surface the change summary
> (added / reprioritized / slipped) as an **interactive card** (`lark_im_card_send`, P4). To build a
> roadmap Base from scratch, delegate to **`base-deploy`** — don't hand-roll schema. Durable roadmap
> docs land in Wiki via `lark_wiki_node_create` (P8).
...
If a roadmap **Lark Base** exists (the system-of-record, P5):
- `lark_base_search` the roadmap table. Note: `lark_base_search` does NOT support `jq` and REQUIRES
  `search_fields` (which field(s) to match against). If you don't know the field names, discover them
  first via `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`. Keep token cost
  low with `select_fields` (e.g. name, status, owner, target, priority, deps) and `limit` instead of `jq`.
- Identify items overdue / at-risk / recently completed by filtering on status + target date.
- Cross-check live delivery state with `lark_task_my` / the task-list endpoint (LARK-RECIPES) for
  items that have backing tasks.
- Surface any items without clear owners or target dates.

If there is no roadmap Base yet:
- Offer to scaffold one by delegating to the **`base-deploy`** skill (a Now/Next/Later or
  quarterly-themes table with status, owner, target, priority, dependency fields).
- Or, if the user just wants a one-off view, ask them to describe / paste / upload the current
  roadmap (any format: list, table, spreadsheet, screenshot, prose).
...
- Offer to draft communication about roadmap changes (delegate the broadcast to the
  **stakeholder-update** skill; post as a card via `lark_im_card_send`, P4)
- Offer to commit status / priority / date changes back to the roadmap Base with
  `lark_base_record_upsert` (`dry_run` first, then confirm, P2)
- Offer to land the updated roadmap as a durable Wiki page via `lark_wiki_node_create` (P8)
- For concrete deliverables, offer to create owner-assigned tasks via `lark_task_create`
  (resolve owner with `lark_contact_search` first, P1)
