...
> Pull context from Lark: related tickets/epics from the project **Base** (`lark_base_search` —
> REQUIRES `search_fields`, no `jq`, narrow with `select_fields`/`limit`, P5),
> prior specs/research/decisions from `lark_doc_search`/`lark_doc_fetch` (jq-projected, P3), meeting
> decisions from `lark_minutes_search` (P6). Author the PRD as a real Lark doc and land it in **Wiki**
> via `lark_wiki_node_create` (P8) — for non-trivial doc editing delegate to the **`lark-doc`** skill
> (DocxXML). Break the spec's P0 requirements into assigned tasks via `lark_task_create` (resolve
> owners with `lark_contact_search`, P1). Design assets (Figma) stay in their own MCP server. To
> deeply explore the problem first, delegate to **product-brainstorming**.
...
- **Project tracker (Base + Task, P5):** `lark_base_search` the backlog/roadmap table for related
  items, existing requirements, acceptance criteria, and dependencies on other work. Note:
  `lark_base_search` does NOT support `jq` and REQUIRES `search_fields` (discover field names via
  `lark_api GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown); narrow with
  `select_fields`/`limit` instead. Cross-check open work with `lark_task_my` / the task-list endpoint (LARK-RECIPES).
- **Knowledge base (Docs/Wiki):** `lark_doc_search` for related research, prior specs, and design
  docs; `lark_doc_fetch` (jq-projected) the ones you'll cite. Pull meeting decisions from
  `lark_minutes_search` (P6).
- **Design:** Figma keeps its own MCP server — pull mockups/wireframes there if connected. There is
  no Lark equivalent; reference it as-is.

Work entirely from what the user provides if these sources are empty — do not ask them to connect
anything, just proceed with available information.
...
### 5. Review and Iterate (Lark-native)

After generating the PRD:
- Ask the user if any sections need adjustment; offer to expand specific sections.
- Land the PRD as a real Lark doc in **Wiki** via `lark_wiki_node_create` (P8); for the body,
  delegate to the **`lark-doc`** skill (DocxXML) — hand-rolling block JSON is brittle.
- Offer to break the P0 requirements into an engineering ticket breakdown: create rows in the
  project Base (`lark_base_record_upsert`, `dry_run` first, P2) and assigned tasks
  (`lark_task_create`, owner `open_id` from `lark_contact_search`, P1).
- Offer to draft a stakeholder pitch — delegate to the **stakeholder-update** skill.
