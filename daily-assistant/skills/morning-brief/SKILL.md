...
> **Companion skills are best-effort.** `approval-triage` / `decision-logger` (governance plugin)
> and `pipeline-review` / `client-followup` (crm-sales plugin) are only spawned when those plugins
> are installed. If a companion is absent, **drop that section silently** — never error, never
> block the brief. Core sections (mail, calendar, IM, tasks) live in this plugin and always run.
