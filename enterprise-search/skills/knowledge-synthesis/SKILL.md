...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). This skill is pure synthesis methodology — it
> operates on results already returned by the `lark_*` reads (IM `lark_im_search`, Mail, Docs/Wiki
> `lark_doc_*`, Minutes `lark_minutes_search`, Tasks/Base `lark_task_my`/`lark_base_search`). Attribute
> each source by its Lark surface + date. When the synthesized output is a **decision recap or digest**
> the user will act on, hand it to the caller to render as an **interactive card** (`lark_im_card_send`,
> P4) rather than plain text. If you need a deeper read of any source to resolve a conflict, delegate
> to that source's owning skill (`lark-doc`, `lark-minutes`, `lark-base`).
...
- One source references another ("as discussed in chat", "per the mail", "see the doc")
...
- Always name the Lark surface (IM, Mail, Docs/Wiki, Minutes, Task, Base) + the tool it came from
- Include the specific location (group/chat, folder, doc/wiki node, Base table)
- Include the date or relative time
- Include the author when relevant (resolved name, not raw `open_id`)
- Include document/thread/meeting titles when available
- For IM (`lark_im_search`), note the group/chat name
- For Mail, note the subject line and sender
- For Docs/Wiki (`lark_doc_*`), note the document title and link
- For Minutes (`lark_minutes_search`), note the meeting title and that it's an AI artifact (P6)
- For Base (`lark_base_search`), note the table/record (the system-of-record, P5)
...
**Do not:**
- List results source by source ("From IM: ... From Mail: ... From Docs: ...")
