...
> **Lark-native note** (depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md)). This is
> pure statistical methodology and stays specialty-external. When invoked inside a Lark workflow, the
> input series can be read from a **Lark Base** (`lark_base_search` — REQUIRES `search_fields`, no `jq`;
> narrow with `select_fields`/`limit`) or **Sheet** (`lark_sheets_read`)
> (P5), and the headline statistic + caveats should be surfaced as an **interactive card**
> (`lark_im_card_send`, P4) and/or written into the report doc via `lark-doc` (P8) — always pairing the
> point estimate with a range, per the uncertainty guidance below.
