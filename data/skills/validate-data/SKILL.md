...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The QA methodology stays as-is. Lark hooks:
> **cross-validation reads from Lark** — when checking key numbers against a known source (Step 4 /
> "Cross-Reference"), pull the canonical figures from the team's **Lark Base** tracker
> (`lark_base_search` — REQUIRES `search_fields`, no `jq`; narrow with `select_fields`/`limit`, P5) or
> **Sheet** (`lark_sheets_read`). **Output as a card** —
> surface the verdict (Ready / Caveats / Needs revision) as an **interactive card** (`lark_im_card_send`,
> P4): colored `header` per verdict, `item` rows for each issue with severity pills, an `actions` footer.
> Land the full validation report in the analysis doc via `lark-doc` (P8). Resolve the analysis author /
> reviewer via `lark_contact_search` before notifying (P1); `dry_run` the card first (P2).
...
Surface the verdict as an **interactive card** (P4) when sharing in chat: `header` template colored by
verdict (green = Ready, orange = Caveats, red = Needs revision), one `item` row per issue with a
severity pill, and an `actions` footer (e.g. *Open report* / *Re-validate*). Use `lark_im_card_send`
with `print_json: true` → `dry_run: true` → send. Land the full report in the analysis doc via the
`lark-doc` skill (P8). Rate the analysis on a 3-level scale:
...
- [ ] **Cross-reference**: Key numbers match other known sources (dashboards, previous reports, finance data). When the canonical source lives in Lark, pull it directly — `lark_base_search` a tracker Base or `lark_sheets_read` a finance Sheet (P5) — and compare.
