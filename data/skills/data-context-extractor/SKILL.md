...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). Warehouse schema discovery (the `~~data warehouse`
> tools) stays specialty-external. Lark hooks: (1) a company's system-of-record is often a **Lark Base**,
> not a warehouse — when so, discover its schema via the **`lark-base`** skill (tables/fields/views) and
> read sample rows with `lark_base_search`, then document those as the entities/metrics/tables (P5).
> (2) **Deliver the generated skill on Lark**, not only as a zip — `lark_drive_upload` the package (P8),
> and publish the human-readable schema docs (entities/metrics) to **Wiki** via `lark_wiki_node_create`
> so analysts find them (P8). Resolve interviewees/owners via `lark_contact_search` (P1).
...
- **PostgreSQL/Redshift**
- **Databricks**
- **Lark Base** (Bitable) — when the team's system-of-record is a Base rather than a warehouse
...
3. **Deliver on Lark (P8)**: `lark_drive_upload` the zip (`dry_run` first, P2), and publish the
   human-readable schema docs (entities/metrics) to **Wiki** via `lark_wiki_node_create` →
   `lark_doc_create` so the team can browse them. Delegate non-trivial doc body to `lark-doc`.
4. Present to user with summary of what was captured
