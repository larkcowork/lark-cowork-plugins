...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). Warehouse profiling stays specialty-external. When
> there's no warehouse, the dataset to profile can be a **Lark Base** (`lark_base_search` — REQUIRES
> `search_fields`, no `jq`; narrow with `select_fields`/`limit`, P5) or **Sheet** (`lark_sheets_read`) —
> treat a Base table's fields/views as the schema. When you
> write up the profile + schema documentation for the team, **land it in Wiki** (`lark_wiki_node_create`
> → `lark_doc_create`, P8) so analysts find it later; delegate non-trivial doc body to `lark-doc`.
...
**If the dataset is a Lark Base or Sheet (no warehouse):**

1. `lark_base_search` the table (the Base's fields are the schema, its views are pre-built filters) —
   it REQUIRES `search_fields` (discover field names via `lark_api` GET
   `/open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown) and does NOT support `jq`, so
   narrow with `select_fields`/`limit`. Or `lark_sheets_read` the range (P3/P5).
2. Load into a working dataset and profile as below. Delegate field/view introspection to `lark-base`.

**If neither:**

1. Ask the user to provide a table name (warehouse), a Lark Base/Sheet, or upload a file
...
When documenting a dataset for team use, author it as a **Wiki page** (`lark_wiki_node_create` →
`lark_doc_create`, P8) so the whole team can find and maintain it — not a local-only markdown file.
Use this structure:
