...
> SQL authoring stays specialty-external. Two Lark hooks: (1) if the user has **no warehouse** but the
> data lives in a **Lark Base/Sheet**, don't write SQL — read it directly with `lark_base_search`
> (it REQUIRES `search_fields` and does NOT support `jq` — narrow with `select_fields`/`limit`, then
> aggregate via the Base data-query endpoint, LARK-RECIPES) or `lark_sheets_read`,
> and say so (P5). (2) Once a query is run, offer to **land the result** in Lark — a Sheet via
> `lark_sheets_append`, a tracker Base via `lark_base_record_upsert` (P5), or a report doc via the
> `lark-doc` skill (P8). `dry_run` writes first (P2).
...
### 6. Offer to Execute and Land Results

If a data warehouse is connected, offer to run the query and analyze the results. If the user wants to
run it themselves, the query is ready to copy-paste.

After execution, offer to **land the output on Lark (P8/P5)** rather than leaving it in chat:
- append to a **Lark Sheet** (`lark_sheets_append`),
- upsert into a tracker **Lark Base** (`lark_base_record_upsert`, the system-of-record),
- or write it into a **report doc** (delegate to the `lark-doc` skill).

`dry_run: true` any write first (P2), and resolve any people referenced in the result via
`lark_contact_search` before tagging them (P1).
