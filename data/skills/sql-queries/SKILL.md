...
> [LARK-RECIPES](../../../connectors/LARK-RECIPES.md)). The warehouse dialects below are
> specialty-external and stay as-is. One caveat for Lark deployments: **Lark Base is not SQL** — if the
> data lives in a Base (no warehouse), don't write SQL against it; read with `lark_base_search` — it
> REQUIRES `search_fields` (the field(s) to match) and does NOT support `jq`, so narrow with
> `select_fields`/`limit` — and aggregate via the Base data-query endpoint (LARK-RECIPES),
> or `lark_sheets_read` for a Sheet (P5). Delegate Base record/field/formula ops to the `lark-base` skill.
