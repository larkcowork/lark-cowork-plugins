...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). This is pure methodology for the Python plotting
> engine (specialty-external). When it's invoked inside a Lark workflow: source DataFrames can come from
> a **Lark Base** (`lark_base_search` — REQUIRES `search_fields`, no `jq`; narrow with
> `select_fields`/`limit`) or **Sheet** (`lark_sheets_read`) (P5), and finished PNG/HTML
> charts should be **landed on Lark** — `lark_drive_upload` (P8), embed in a doc via `lark-doc`, or
> attach to a card via `lark_im_card_send` (P4). For a *live* native dashboard inside Lark (KPI/chart
> blocks bound to a Base), delegate to **`base-viz`** instead of static charts.
