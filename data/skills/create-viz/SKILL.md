...
> [LARK-FUSION](../../../connectors/LARK-FUSION.md)). The Python plotting engine stays specialty-external.
> The **Lark layer** is data-in / chart-out: when there's no warehouse, the source can be a **Lark Base**
> (`lark_base_search` — REQUIRES `search_fields`, no `jq`; narrow with `select_fields`/`limit`) or
> **Sheet** (`lark_sheets_read`) → DataFrame (P5). After saving the PNG, **land it
> in Lark** rather than leaving it loose: `lark_drive_upload` the image (P8), then either embed it via the
> `lark-doc` skill in a report, or attach it to a `lark_im_card_send` card (P4, `image` element). Resolve
> any recipient via `lark_contact_search` (P1); `dry_run` the upload (P2).
...
**If the source is a Lark Base or Sheet (no warehouse):**
1. `lark_base_search` — pass `search_fields` (the field(s) to match; discover names via `lark_api` GET
   `/open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown); it does NOT support `jq`, so
   narrow with `select_fields`/`limit`. Or `lark_sheets_read` a named range (P5).
...
### 6. Save, Present, and Land on Lark

1. Save the chart as a PNG file with descriptive name
2. Display the chart to the user
3. **Land it in Lark (P8)**: `lark_drive_upload` the PNG (`dry_run` first, P2). Then embed it in a
   report doc via the `lark-doc` skill, or push it to chat as a card image (`lark_im_card_send` with an
   `image` element, P4) — resolve the recipient via `lark_contact_search` (P1).
4. Provide the code used so they can modify it
5. Suggest variations (different chart type, different grouping, zoomed time range)
