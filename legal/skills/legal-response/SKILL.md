...
> - **Templates** live in Wiki/Docs/Base, not just `legal.local.md` — find them with `lark_doc_search`, read with `lark_doc_fetch` (`jq` projection, P3), or a template Base via `lark_base_search` (REQUIRES `search_fields`; no `jq` — read `.data.records[]`, narrow with `select_fields`/`limit`; discover field names via `lark_api` fields endpoint if unknown).
> - **Never auto-send** (P2). Produce the response, then create a **Lark Mail draft** with `lark_mail_draft_create` (or `lark_mail_send` *without* `confirm_send`, which only drafts) so a human eyeballs it in the Mail UI first. Resolve the recipient to `open_id`/address via `lark_contact_search` (P1).
> - **Log every inquiry + response** to a Lark Base (P5, `lark_base_record_upsert`) — request type, requester, regulation, deadline, status — so deadlines (DSR 30/45-day, hold acknowledgments) are tracked. Set a Lark Task / calendar reminder for the deadline (`lark_task_create` / `lark_calendar_create`).
> - **Escalation triggers → real human gate** (P7): when a trigger fires, don't draft a final reply; create a Lark **approval instance** (`lark_api` approval recipe / **`lark-approval`** skill) routed to senior/outside counsel, or post a decision **card** (`lark_im_card_send`, P4) with Approve / Send-to-counsel buttons. For litigation-hold notices, fan out the hold as Tasks to each custodian and require acknowledgment.
...
4. **Recommend**: Suggest the appropriate escalation path (senior counsel, outside counsel, specific team member). Resolve them via `lark_contact_search` (P1) and route a real Lark **approval instance** (P7) or a decision **card** (P4) to that person, rather than relying on the user to forward.
5. **Offer**: Provide a draft for counsel review (clearly marked as "DRAFT - FOR COUNSEL REVIEW ONLY") as a **Lark Mail draft** (`lark_mail_draft_create`, never auto-send) rather than a final response.
...
- Always present the draft response for user review before suggesting it be sent (P2 — never auto-send)
- Offer to create a Lark Mail draft with the response via `lark_mail_draft_create` (or `lark_mail_send` without `confirm_send`)
- Track response deadlines: log to the inquiry Base, create a Lark Task (`lark_task_create`) and/or calendar reminder (`lark_calendar_create`) for each deadline
