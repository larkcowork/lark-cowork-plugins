...
> **Lark-native execution** (see [CONNECTORS.md](../../CONNECTORS.md) and the depth core: [LARK-PATTERNS](../../../connectors/LARK-PATTERNS.md), [LARK-RECIPES](../../../connectors/LARK-RECIPES.md), [LARK-FUSION](../../../connectors/LARK-FUSION.md)).
> - This two-phase flow (before: context; after: action items → tasks) duplicates the installed **`meeting-prep`** skill — prefer delegating to it; for participant 360s delegate to **`contact-360`**, for 1:1s **`one-on-one-prep`**. Frame it for legal and hand off execution.
> - Gather context with real tools, most `jq`-projected (P3): event details → `lark_calendar_agenda` (`.data[]`); participant resolution + roles → `lark_contact_search` (P1, `.data.users[]`); prior threads → `lark_im_search` (`.data.messages[]`) and `lark_api` mail search; relevant docs/agreements → `lark_doc_search` (`.data.results[]`) + `lark_doc_fetch`; contract/matter status → `lark_base_search` (P5 — REQUIRES `search_fields`, NO `jq`; narrow with `select_fields`/`limit`, read `.data.records[]`); past meetings + AI artifacts → `lark_vc_search` / `lark_minutes_search` (`.data.items[]`).
> - **Action items: pull from Minutes, don't re-derive** (P6). After the meeting, `lark_minutes_search(participant_ids="me")` for the AI action items; for each, resolve the owner (P1) and `lark_task_create` (`dry_run` first, P2).
> - **Deliver the brief as an interactive card** (P4) via `lark_im_card_send` — `header` with the meeting, `panel` per section, `item` rows for open issues/decisions-needed. Land the durable brief + notes in **Wiki** (`lark_wiki_node_create`, P8). Distribute action items as a card with owner pills, not a plain email.
...
After the meeting:
1. **Distribute action items** as an interactive card (`lark_im_card_send`, P4) to the meeting chat, and create a Lark Task per item (`lark_task_create`, owner resolved via `lark_contact_search`, `due` set, `dry_run` first, P2)
2. **Set calendar reminders** for deadlines via `lark_calendar_create`
3. **Update relevant systems** — write outcomes to the matter/risk-register Lark Base (`lark_base_record_upsert`, P5)
4. **File meeting notes** in **Wiki** (`lark_wiki_node_create`, P8) — not as a chat message
5. **Flag urgent items** that need immediate attention (red card `header` / danger-type buttons)
