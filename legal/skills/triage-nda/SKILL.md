...
> - **Ingest the NDA**: Lark Doc URL → `lark_doc_fetch` (`jq`-projected, P3); Drive file → `lark_doc_search` then fetch; upload → `lark_drive_upload`.
> - **NDA playbook** (mutual/term/carveout rules) lives in Wiki/Docs — find with `lark_doc_search`, read with `lark_doc_fetch`. Log each triaged NDA to the contract/NDA-tracking **Lark Base** (P5, `lark_base_record_upsert`: parties, type, term, classification, status).
> - **Surface the GREEN/YELLOW/RED classification as an interactive card** (P4) via `lark_im_card_send` — colored `header` by classification, `item` rows per flagged issue, `actions` row (e.g. *Approve & route to signature*, *Send to counsel*). `print_json` → `dry_run` → send.
> - **Routing is a real action, not just a label** (P7): GREEN → hand to `signature-request`; YELLOW/RED → resolve the reviewer (`lark_contact_search`, P1) and create a Lark **approval instance** (`lark_api` approval recipe / **`lark-approval`** skill) or a Lark Task with the issue summary. Never auto-send a counterproposal — draft it (P2).
...
- Identify the specific reviewer and resolve them via `lark_contact_search` (P1); route via a Lark **approval instance** (P7) or a Lark Task carrying the issue summary
- Include a brief summary of issues suitable for the reviewer to quickly understand the key points (as the card body / task description)
- If the organization has a standard form NDA, recommend sending it as a counterproposal for RED-classified NDAs — as a **Lark Mail draft** (`lark_mail_draft_create`, never auto-send, P2)
