...
> - There is no native CLM/CRM product — the **contract & vendor register is a Lark Base** (P5). Search it with `lark_base_search` (filter by vendor) — `lark_base_search` REQUIRES `search_fields` (the Bitable API mandates which field(s) to match; if field names are unknown discover them via `lark_api` `GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields`) and does NOT support `jq`; narrow with `select_fields`/`limit` and read `.data.records[]`. The external CLM/CRM MCP is used only if separately connected.
> - Per-source tools: agreements/drafts in **Drive/Docs** → `lark_doc_search` + `lark_doc_fetch`; correspondence → `lark_api` mail search; team chatter → `lark_im_search`; vendor contact → `lark_contact_search` (P1). Project every read with `jq`.
> - **Present the consolidated status + gap analysis as an interactive card** (P4) via `lark_im_card_send` — `header` with the vendor, `item` rows for each agreement (with a side button to open it), a `panel` for gaps (missing DPA/MSA/SOW). `print_json` → `dry_run` → send.
> - **Upcoming expirations / renewal deadlines** (within 90 days) → create Lark Tasks (`lark_task_create`, owner resolved via `lark_contact_search`, `dry_run` first, P2) and/or calendar reminders (`lark_calendar_create`) so the deadline is tracked natively, not just reported.
...
Search for the vendor across all available connected systems, in priority order. Lark mapping: CLM/CRM → **Lark Base** (`lark_base_search` — pass the required `search_fields`; no `jq`), Email → `lark_api` mail search, Documents → `lark_doc_search` + `lark_doc_fetch`, Chat → `lark_im_search`, contacts → `lark_contact_search`. Use the external CLM/CRM MCP only if one is separately connected.

#### CLM (Contract Lifecycle Management) -- Lark Base (or external CLM MCP if connected)
Search the contract register Base (`lark_base_search`, passing the required `search_fields` — e.g. the vendor-name field; discover field names via `lark_api` fields endpoint if unknown) for all contracts involving the vendor:
- Active agreements
...
#### Chat (e.g., Lark IM, Teams) -- If Connected
