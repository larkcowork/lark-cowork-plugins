...
> - **Ingest the contract**: if it's a Lark Doc URL, read it with `lark_doc_fetch` (project with `jq`, e.g. `.content[:8000]`, P3); if it's a Lark Drive file, locate via `lark_doc_search`; uploads go through `lark_drive_upload`. There is no native CLM — query the external CLM MCP if connected, else treat the contract register as a Lark Base.
> - **Playbook** (standard positions, acceptable ranges, escalation triggers) lives in Wiki/Docs — find with `lark_doc_search`, read with `lark_doc_fetch`. For DocxXML editing of redlines, delegate to **`lark-doc`** (block JSON by hand is brittle).
> - **Track the review in a Lark Base** (P5): write the contract record + risk flags via `lark_base_record_upsert` (`dry_run` first, P2); read prior reviews via `lark_base_search` (REQUIRES `search_fields`; no `jq` — read `.data.records[]`, narrow with `select_fields`/`limit`; discover field names via `lark_api` fields endpoint if unknown). Scaffold the schema with **`base-deploy`**, not by hand.
> - **Surface the GREEN/YELLOW/RED findings as an interactive card** (P4) via `lark_im_card_send` — colored `header` by overall risk, `item` rows per RED/YELLOW clause with a side button (e.g. *Escalate*, *Accept fallback*), `panel` for the negotiation strategy. `print_json` → `dry_run` → send.
> - **CLM routing / approval** (Step 8): RED contracts and over-threshold value should trigger a real Lark **approval instance** (P7, `lark_api` / **`lark-approval`**) routed to the resolved approver (`lark_contact_search`, P1) — not a chat "ok". Capture follow-up redline tasks as Lark Tasks. Land the executed review memo in **Wiki** (`lark_wiki_node_create`, P8).
...
- **File upload**: PDF, DOCX, or other document format → ingest via `lark_drive_upload`
- **URL**: a Lark Doc/Wiki link (read with `lark_doc_fetch`, `jq`-projected), a Lark Drive file (find via `lark_doc_search`), or a contract in an external CLM/cloud-storage MCP
...
If a Contract Lifecycle Management system is connected via its own MCP, use it. Otherwise approximate CLM natively with Lark:
- Write the contract + risk level to the contract Base (`lark_base_record_upsert`, P5).
- For routing that needs a human gate (RED flags, over-threshold value), create a real Lark **approval instance** (P7, `lark_api` approval recipe / **`lark-approval`** skill), routed to the resolved approver (`lark_contact_search`, P1: standard approval, senior counsel, or outside counsel).
- Note any required approvals based on contract value or risk flags as `actions` buttons on the findings card (P4).
