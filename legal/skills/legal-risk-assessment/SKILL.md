...
> - The **risk register is a Lark Base** (P5), the system of record for every entry below. Read prior risks with `lark_base_search` — it REQUIRES `search_fields` (the Bitable API mandates which field(s) to match; discover field names via `lark_api` `GET /open-apis/bitable/v1/apps/{base}/tables/{table}/fields` if unknown) and does NOT support `jq`; narrow with `select_fields`/`limit` and read `.data.records[]`. Write each assessment with `lark_base_record_upsert` (`dry_run` first, P2). Scaffold the register schema (Risk ID, Severity, Likelihood, Score, Level, Owner, Status, Review Date) with **`base-deploy`**, not by hand; CRUD/formula depth → **`lark-base`**.
> - Pull supporting context (prior memos, deal docs, meeting decisions) from Docs/Wiki (`lark_doc_search`/`lark_doc_fetch`) and Minutes (`lark_minutes_search`, P6) rather than re-deriving.
> - **Surface ORANGE/RED risks as an interactive card** (P4) via `lark_im_card_send` — colored `header` by risk level, `item` rows per risk with a side *Escalate* button, `panel` for mitigation options. `print_json` → `dry_run` → send.
> - **Escalation (ORANGE/RED) is a real human gate** (P7): resolve the approver/senior counsel via `lark_contact_search` (P1) and route a Lark **approval instance** (`lark_api` approval recipe / **`lark-approval`** skill); capture each mitigation step in the Monitoring Plan as a Lark Task with a `due` (`lark_task_create`) and a review-date calendar reminder (`lark_calendar_create`). Land the full risk memo in **Wiki** (`lark_wiki_node_create`, P8), marked privileged.
...
For tracking in the team's risk register — this is a **Lark Base** record; write it via `lark_base_record_upsert` (P5, `dry_run` first) mapping each field below to a Base column, and resolve the Owner to an `open_id` via `lark_contact_search` (P1):
